...
import { addDataKrs } from "@/src/Redux/Admin/KrsDataSlice";
...
export default function AddKrs({ open, handleClose }: AddDataStudentsProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/krs/create",
        getFormValue
      );
      ...
      if (response.data.status === "success") {
        const updateFormValue = {
          ...getFormValue,
          id: response.data.data.newData.id,
        };
        dispatch(addDataKrs(updateFormValue));
        handleClose();
        setAlertMessage({
          ...alertMessage,
          isOpen: true,
          message: "Successfully added KRS data",
        });
      }
    } catch (error) {
      console.error(error);
